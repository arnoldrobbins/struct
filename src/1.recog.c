...
int type, ifflag;		/* do whatever is needed for this statement */
{
	...
	if (nlabs > 3)
		...
		sp = 3;
	arctype = challoc(sizeof(*arctype) * sp);
	arclab = challoc(sizeof(*arclab) * sp);
	for (i = 0; i < endbuf; i++) {
		if (buffer[i] == '~')
			buffer[i] = ' ';
	}
	...
	case STLNVX:
		if (DEFINED(stflag) && !ifflag && (label(0) == implicit)) {
			...
			num =
			    makenode(STLNVX, !ifflag, !ifflag, label(0), 1,
				     arctype, arclab);
			if (!ifflag) {
				stflag = num;
				BEGCODE(num) = comchar + 1 - rtnbeg;
				if ((unsigned) (BEGCODE(num)) !=
				    comchar + 1 - rtnbeg)
					faterr("program too long", "", "");
				ONDISK(num) = endline - endcom;
				CODELINES(num) = 1;
	...
	case arithif:
		if (label(1) == label(2) || label(1) == 0L)
			makeif(1, label(0), concat(pred, " > 0"), label(3),
			       label(2));
		else if (label(1) == label(3) || label(3) == 0L)
			makeif(1, label(0), concat(pred, " == 0"),
			       label(2), label(1));
		else if (label(2) == label(3) || label(2) == 0L)
			makeif(1, label(0), concat(pred, " < 0"), label(1),
			       label(3));
		else {
			makeif(1, label(0), concat(pred, " < 0"), label(1),
			       implicit);
			makeif(1, implicit, concat(pred, " == 0"),
			       label(2), label(3));
		}
		break;
	...
	if (ifflag || DEFINED(loophead) || type != STLNVX)
		stflag = UNDEFINED;


	chfree(arctype, sizeof(*arctype) * sp);
	chfree(arclab, sizeof(*arclab) * sp);
	if (debug) {
		...
		switch (type) {
		case RETVX:
			fprintf(debfd, "return");
			break;
		case STOPVX:
			fprintf(debfd, "stop");
			break;
		case contst:
			fprintf(debfd, "continue");
			break;
		case ungo:
			fprintf(debfd, "uncond. goto");
			break;
		case COMPVX:
			fprintf(debfd, "comp. goto");
			break;
		case ASGOVX:
			fprintf(debfd, "ass. goto, labs");
			break;
		case ASVX:
			fprintf(debfd, "label assignment");
			break;
		case STLNVX:
			fprintf(debfd, "simple statement");
			break;
		case arithif:
			fprintf(debfd, "arith if");
			break;
		case DOVX:
			fprintf(debfd, "do ");
			break;
		case FMTVX:
			fprintf(debfd, "format st");
			break;
		case IOVX:
			fprintf(debfd, "IOVX statement ");
			break;
		case entry:
			fprintf(debfd, "entry statement ");
			break;
		}
		fprintf(debfd, "\n%s\n", buffer);
	}
...
int nest;
{
	...
	if (!DEFINED(nest)) {	/* not nested */
		if (labe != implicit)
			connect(implicit, labe);	/* labe pts to next node */
	} else {		/* nested */
		y = ARC(nest, 0);
